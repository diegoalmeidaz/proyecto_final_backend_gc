...
exports.updateUser = async (req, res) => {
  ...
  try {
    const { rows } = await db.query('UPDATE users SET name = $1 WHERE user_id = $2 RETURNING *', [name, user_id]);
...
exports.getUserInfo = async (req, res) => {
  try {
    ...
    const { rows } = await db.query(`
      SELECT users.user_id, users.username, users.email, users.name, users.lastname, users.address, users.phone, roles.role_name
      FROM users
    ...
    console.log('Error en getUserInfo:', error.message);
    return res.status(500).json({
      error: error.message,
    });
  }
